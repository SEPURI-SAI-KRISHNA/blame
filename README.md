...
![clicking the wrong number lights up the rows that made it](docs/demo.gif)
...
narrowed and never presented as exact — see [docs/accuracy.md](docs/accuracy.md).

Capture costs about **2x** wall time and **5 ms per step** on a million rows
([benchmarks](docs/benchmarks.md)). Tested on pandas 2.0.3, 2.2.3 and 3.0.5;
the eight pandas documentation tutorials run under it with no exceptions, no
changed results and no approximate steps ([validation](docs/validation.md)).
...
that is the bug report worth filing. [Roadmap](ROADMAP.md).
