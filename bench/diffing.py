...
def run(n: int, bump: float):
    ...
    customers = pd.DataFrame({"cust": np.arange(500),
                              "region": rng.choice(list("nsew"), 500)})
...
def main() -> None:
    print(f"{'source rows':>12} {'diff':>9} {'changed out':>12} {'explained':>10} "
          f"{'changed in':>11}")
    for n in (10_000, 100_000, 1_000_000):
        ...
        changed_in = len(d.source_summary["orders"].split("changed")[0].split()[-1])
        print(f"{n:>12,} {elapsed * 1000:>8.0f}ms {len(d.changes):>12} "
